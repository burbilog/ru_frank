...
1. **Промпт для LLM:** Включает готовый промпт (`PROMPT.txt`), который вы копируете в любую LLM-модель вместе с японским текстом. Модель добавляет фуригану (произношение) в скобках после каждого слова с иероглифами
2. **Скрипт для LibreOffice:** Автоматически находит текст с фуриганой в скобках и преобразует его в правильное ruby-форматирование (запись произношения хираганой над иероглифами)
3. **Универсальность:** Работает как с японскими полноширинными （） так и с обычными ASCII () скобками
...
3. **Обязательная проверка:** Подайте попросите модель перепроверить свою работу:
